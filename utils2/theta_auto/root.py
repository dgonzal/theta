...
def build_model_from_roothistograms(histograms, histogram_filter = lambda s: True, root_hname_to_convention = lambda s: s, transform_histo = lambda h: h, include_mc_uncertainties = False):
    """
    Build a multi-channel model based on template morphing from histograms in a root histogram
    Documantation see function above build_model_from_rootfile
    ...
    for hexternal in histograms:
        ...
        if uncertainty is not None: 
            h_new = '%s__%s__%s__%s' % (observable, process, uncertainty, direction)
        else: h_new = '%s__%s' % (observable, process)
        histos[h_new] = h_mine
